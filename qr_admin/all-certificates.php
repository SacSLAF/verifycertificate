                                    ...
                                    <div class="table-responsive">
                                        <table id="multi-filter-select" class="display table table-striped table-hover">
                                            ...
                                            <tbody>
                                                <?php
                                                if (($role == 'super_admin')) {
                                                    $query = "SELECT `id`, `certificate_id`, `name`, `service_no`, `rank`, `date_of_issue`, `issuing_authority_name`,`certificate_uuid`,`directorate_id`, `verification_status`,is_active FROM certificates";
                                                    $stmt = mysqli_prepare($conn, $query);
                                                } else {
                                                    if ($user_type == '1' || $user_type == '2') {
                                                        $query = "SELECT `id`, `certificate_id`, `name`, `service_no`, `rank`, `date_of_issue`, `issuing_authority_name`,`certificate_uuid`,`directorate_id`, `verification_status`,is_active FROM certificates WHERE institute_id = ?";
                                                        $stmt = mysqli_prepare($conn, $query);
                                                        mysqli_stmt_bind_param($stmt, "i", $institute);
                                                    } else {
                                                        $query = "SELECT `id`, `certificate_id`, `name`, `service_no`, `rank`, `date_of_issue`, `issuing_authority_name`,`certificate_uuid`,`directorate_id`, `verification_status`,is_active FROM certificates WHERE directorate_id = ?";
                                                        $stmt = mysqli_prepare($conn, $query);
                                                        mysqli_stmt_bind_param($stmt, "i", $directorate_id);
                                                    }
                                                }
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    ...
                                                    if (!$is_admin && $row['directorate_id'] == $directorate_id) {
                                                        echo '<a href="make-certificate-v1.php?id=' . htmlspecialchars($row['id']) . '&action=edit" class="btn btn-info btn-sm" title="Edit">';
