...
if ($action == 'add') {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($type == 3 && empty($directorate)) {
            die('Directorate is required for Leaving Service Certificate users');
        }
        ...
        $query = "INSERT INTO users (username, password, training_institute,camp,directorate,created_at, `role`, `type`) VALUES (?, ?, ?,?,?, ?, ?, ?)";
        ...
        if ($stmt = mysqli_prepare($conn, $query)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                header("Location: ../create-users.php?msg=4");
            } else {
                // echo "Error inserting certificate details: " . mysqli_error($conn);
                header("Location: ../create-users.php?msg=6");
            }
